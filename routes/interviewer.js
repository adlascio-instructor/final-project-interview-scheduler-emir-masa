...
router.get("/interviewers/:dayId", (req,res)=>{
    const pool = new Pool(dbCredentials);
    pool
        .query(`SELECT available_interviewers.id, interviewers.name FROM available_interviewers INNER JOIN interviewers ON available_interviewers.interviewer_id = interviewers.id`) 
        .then((result) => result.rows)
